...
const InvoiceFormModal = ({ isOpen, onClose, onSave, invoice, prefillData, formType = 'invoice' }) => {
    ...
    const [quickProductPrefill, setQuickProductPrefill] = useState({ name: '', price: 0 });

    // ... (existing code: getLabels)
    ...
    const openQuickProduct = (index, name, price) => {
        setQuickProductIndex(index);
        setQuickProductPrefill({ name, price });
    ...
    const handleQuickProductSave = (newProduct) => {
        ...
        // Select it in the triggering row
        if (quickProductIndex !== null) {
            handleItemChange(quickProductIndex, 'product', newProduct._id);
            // Also update the price if needed, although handleItemChange does it, 
            // but we might want to prioritize the invoice price or product price.
            // Let's stick to handleItemChange logic which pulls from product.
        }
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (formType === 'order') {
            // Map fields for orders
            payload = {
                orderNumber: formData.invoiceNumber,
            ...
            // Map fields for offers
            payload = {
                offerNumber: formData.invoiceNumber,
            ...
            // For invoices
            payload.products = payload.items.map(item => ({
                product: item.product,
            ...
            if (formData.type === 'sale') {
                payload.customerOrSupplier = formData.customer;
                payload.partnerModel = 'Customer';
            } else {
                payload.customerOrSupplier = formData.customer;
                payload.partnerModel = 'Supplier';
            }
    ...
    return (
        <>
                                                    ...
                                                    <tr key={index}>
                                                        <td className="px-4 py-2">
                                                            <div className="flex gap-2">
                                                                ...
                                                                <button
                                                                    type="button"
                                                                    onClick={() => openQuickProduct(index, item.placeholderName, item.unitPrice)}
            ...
            <QuickProductModal
                ...
                prefillName={quickProductPrefill.name}
                prefillPrice={quickProductPrefill.price}
            />
        </>
    );
};
